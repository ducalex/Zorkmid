...
static void select_delta_tables(TrueMotion1Context *s, int delta_table_index)
{
    ...
    for (i = 0; i < 8; i++)
    {
        s->ydt[i] = ydts[delta_table_index][i];
        s->cdt[i] = cdts[delta_table_index][i];
        s->fat_ydt[i] = fat_ydts[delta_table_index][i];
        s->fat_cdt[i] = fat_cdts[delta_table_index][i];
...
int truemotion1_decode_header(TrueMotion1Context *s)
{
    int i;
    int width_shift = 0;
    int new_pix_fmt;
    ...
    if (compression_types[header.compression].algorithm == ALGO_RGB24H)
    {
        ...
        new_pix_fmt = 16; // RGB565 is supported as well
...
void truemotion1_decode_16bit(TrueMotion1Context *s)
{
    int y;
...
void truemotion1_decode_24bit(TrueMotion1Context *s)
{
    int y;
    int pixels_left; /* remaining pixels on this line */
    unsigned int predictor_pair;
...
int truemotion1_decode_frame(avi_file_t *avctx, void *pkt, int pkt_sz)
{
    ...
    // int32_t w = avctx->header.width;
    // int32_t h = avctx->header.height;

    //memset(avctx->frame,0,w*h*4);
